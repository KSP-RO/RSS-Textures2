...
//   --sources <tag>  release tag on KSP-RO/RSS-Textures-Source ("latest" works)
...
//   --allow-height-downscale
//                    permit downscaling a heightmap from a larger one. Off by
//                    default: measured against the shipped pack this moves
//                    terrain by tens of metres. Use TopoConv on the DEM.
...
function parseArgs(argv) {
  const o = {
    ...
    allowHeightDownscale: false, report: null,
  };
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === '--allow-height-downscale') o.allowHeightDownscale = true;
...
async function convertMap(opts, manifest, ctx, bodyName, kind, map) {
  ...
  // Heightmaps are not downscaled from a larger heightmap.
  //
  // Measured against the shipped pack: halving the 8192 MoonHeight and
  // comparing to the shipped 4096 gives a mean difference of 183 units (56 m),
  // max 3539 (1075 m), for box and median alike, against the map's own
  // neighbour-to-neighbour variation of 366 units. Those are two independent
  // resamplings of the same terrain, not one derived from the other. TopoConv
  // resamples the source DEM at each target width; that is what shipped.
  //
  // Downscaling anyway would move terrain tens of metres and put landed craft
  // underground, so it takes an explicit flag.
  if (kind === 'Height' && src.width > target.width && !opts.allowHeightDownscale) {
    return {
      mapName,
      skipped: 'heightmap needs ' + target.width + 'px but source is ' + src.width +
        'px; run TopoConv against the DEM instead (--allow-height-downscale to override)',
      notes: src.notes,
    };
  }
  ...
  if (kind === 'Height') {
    ...
    if (g.width > target.width) g = resampleTo16(g.data, g.width, g.height, target.width, filter);
...
async function main() {
  ...
  if (!opts.from && !opts.sources) throw new Error('need --sources <tag> or --from <set>');
