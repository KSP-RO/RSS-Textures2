...
//   --max-drift <m>    fail if --compare shows a mean drift above this many
//                      metres. This is the release gate; see below.
...
//
// --max-drift is what makes this safe to run in the release build. Regenerating
// a heightmap moves terrain, and terrain that moves puts landed craft
// underground. topo30.raw as measured sits 2690 m below the shipped map over
// Antarctica because it lacks the ice-surface fill. So the build compares
// against the shipped heightmap and refuses to publish a map that has drifted,
...
function parseArgs(argv) {
  const o = {
    ...
    compare: null, maxDrift: null, list: false, dryRun: false,
  };
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === '--max-drift') o.maxDrift = Number(argv[++i]);
...
async function main() {
  ...
  for (const b of built) {
    ...
    const limit = opts.maxDrift ?? 1;
    if (c.meanMetres > limit) {
      console.log('      ' + (opts.maxDrift == null ? 'WARNING' : 'DRIFT') +
        ': differs from the shipped heightmap by more than ' + limit + ' m on average.');
      console.log('      Publishing it moves terrain under anything already landed.');
      if (opts.maxDrift != null) drifted.push({ map: b.map, mean: c.meanMetres, max: c.maxMetres });
    }
  }
  console.log('\ngenerated ' + built.length + ', skipped ' + skipped.length);
  for (const s of skipped) console.log('  ' + s.map.padEnd(16) + s.why);

  // The release gate. Without this, a DEM that disagrees with the shipped
  // terrain would be packaged and only noticed by whoever's base sank.
  if (drifted.length) {
    throw new Error('\n' + drifted.length + ' heightmap(s) drifted past --max-drift ' +
      opts.maxDrift + ' m:\n' +
      drifted.map((d) => '  ' + d.map + '  mean ' + d.mean.toFixed(1) +
        ' m, max ' + d.max.toFixed(0) + ' m').join('\n') +
      '\nRefusing to publish regenerated terrain that does not match what shipped.\n' +
      'Either the DEM is not the one the shipped maps came from, or this is an\n' +
      'intended terrain change - which is a decision to coordinate with RSS, not\n' +
      'something a release picks up on its own.');
  }
}
